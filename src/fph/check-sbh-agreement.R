...
library(lubridate)
#' Inputs
sbh <- readRDS("./gen/sbh/temp/sbh-qsecover-qwsec01.rds")
...
length(unique(sbh$level_1_id)[!(unique(sbh$level_1_id) %in% unique(fph$level_1_id))]) # 7008
...
length(unique(sbh_hadc$level_1_id)[!(unique(sbh_hadc$level_1_id) %in% unique(fph$level_1_id))]) # 19
# 19 of those in sbh are not in fph when they should be
...
nrow(subset(datall, (q208 != 0 | q212 != 0) & is.na(n_preg))) # 19
...
nrow(subset(datall, is.na(q208))) # 0
nrow(subset(datall, is.na(q212))) # 0
nrow(subset(datall, (q208 != 0 | q212 != 0) & is.na(n_bth))) # 19
...
nrow(subset(datall, q208 == 0 & q212 == 0 & is.na(n_bth))) # 6989
nrow(subset(datall, q208 == 0 & q212 == 0 & is.na(n_preg))) # 6989
# sometimes they do
nrow(subset(datall, q208 == 0 & q212 == 0 & !is.na(n_bth))) # 59
nrow(subset(datall, q208 == 0 & q212 == 0 & !is.na(n_preg))) # 59
...
nrow(subset(dat, is.na(n_bth))) # 19
nrow(subset(dat, q208 == n_bth)) # 18607
nrow(subset(dat, q208 != n_bth)) # 377
...
nrow(subset(dat, is.na(n_preg))) # 19
nrow(subset(dat, q212 == n_preg)) # 18578
nrow(subset(dat, q212 != n_preg)) # 406
...
# not in fph
nrow(subset(dth, is.na(n_dth))) # 7008
# among those who had children
sbh_hadc <- subset(sbh, q208 != 0 | q212 != 0 )
