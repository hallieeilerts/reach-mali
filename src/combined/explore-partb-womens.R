...
hhd <- read_sav("./data/instat-20250623/PartA_Household_census_EBM.sav")
names(hhd) <- tolower(names(hhd))
hhd %>%
  group_by(hhd$hh_geo1_nom) %>%
  summarise(cluster = n_distinct(hh_ea))


dat <- read_sav("./data/instat-20250623/PartB-Womens_questionnaire_EBM.sav")
names(dat)
var_info <- tibble(
  name = names(dat),
...
nrow(dat) # 26180
#View(dat[,grepl("q2",names(dat))])

# how many SE per region?
dat %>%
  group_by(qlregion) %>%
  summarise(district = n_distinct(qdistrict),
            commune = n_distinct(wi3),
            cluster = n_distinct(w_grappe))


# dat <- read_sav("./data/instat-20250623/PartA_Household_census_EBM_Décès.sav")
# names(dat)
# dat <- read_sav("./data/instat-20250623/PartA_Household_census_EBM_MEMBRE.sav")
# names(dat)
# dat <- read_sav("./data/instat-20250623/PartC-Household_questionnaire1_EBM.sav")
# names(dat)
# dat <- read_sav("./data/instat-20250623/PartC-Household_questionnaire2_EBM.sav")
# names(dat)

# none of these are unique identifiers
