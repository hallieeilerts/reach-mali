...
library(ggplot2)
#' Inputs
dat <- readRDS("./gen/combined/temp/fph-clean-aod.rds")
################################################################################

# drop those missing indate
# the new combined files have some that are missing intdate
dat <- subset(dat, !is.na(qintdate))
...
nrow(subset(dat, !is.na(aadd) & is.na(q220m) | q220m == 98)) # 21054
nrow(subset(dat, !is.na(aadd) & is.na(q220d) | q220d == 98)) # 24054
nrow(subset(dat, !is.na(aadd) & (is.na(q220d) | q220d == 98) & (is.na(q220m) | q220m == 98))) # 2797
...
dat_knw <- subset(dat, !(recnr %in% dat_unk$recnr))
...
dat_unk_other <- subset(dat_unk, !(recnr %in% dat_imp_d$recnr))
...
nrow(subset(dat2, is.na(dob))) # 0
nrow(subset(dat2, event == 1 & is.na(dod))) # 0
nrow(subset(dat2, event == 1 & !is.na(dod))) # 6911
nrow(subset(dat2, event == 0 & is.na(dod))) # 68095
table(dat2$event, useNA = "always")
...
dat2 <- dat2 %>%
  select(-c(qintdate_y, qintdate_m, qintdate_d, 
            q220m_imp, q220d_imp, q220_date_stub, max_day))

# Save --------------------------------------------------------------------

saveRDS(dat2, "./gen/combined/temp/fph-impute-dob.rds")
