################################################################################
#' @description Exploration of combined file for women's questionnaire
#' @return None
################################################################################
#' Clear environment
rm(list = ls())
...
library(haven)
#' Inputs
dat <- read_sav("./data/instat-20250623/PartB-Womens_questionnaire_historique_grossesse_EBM.sav")
################################################################################
...
# recode empty strings or blank spaces on character columns only
dat <- dat %>%
  mutate(across(where(is.character), ~ ifelse(trimws(.) == "", NA, .)))

# create augmented q223 (final pregnancy outcome)
dat$q223_aug <- dat$q223

# pregnancy outcome was sometimes reported in q218 which is supposed to be babies name
dat %>% 
  group_by(q218) %>%
  summarise(n = n()) %>%
  arrange(-n) %>%
...
nrow(subset(dat, is.na(q223_aug))) # 289
nrow(subset(dat, is.na(q223_aug) & !is.na(q216))) # 270
...
# this only helped fill in 289-274 = 15
...
nrow(subset(dat, q224 == 1 & q223_aug != 1)) # 1
data.frame(subset(dat, q224 == 1 & q223_aug != 1))
# in this cases, both children listed as living with the respondent (q226 == 1) and they dont have a age at death (q228)
...
# q223_aug is still missing in 274 cases
...
table(subset(dat, q223_aug == 2)$q221n) # there is one case of 6 months
...
saveRDS(dat, "./gen/combined/temp/fph-clean.rds")
