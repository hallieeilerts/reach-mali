...
library(ggplot2)
#' Inputs
dat <- readRDS("./gen/combined/temp/fph-recover-doby.rds")
################################################################################
...
# still alive
dat$event <- ifelse(as.character(haven::as_factor(dat$q224)) == "Non", 1, 0)
...
nrow(subset(dat, !is.na(aadd))) # 6916
nrow(subset(dat, !is.na(aadm))) # 6916
nrow(subset(dat, !is.na(aady))) # 6916
...
nrow(id_drop1) # 7
nrow(dat) # 79701
dat <- subset(dat, !(recnr %in% id_drop1$recnr))
nrow(dat) # 79694

nrow(subset(dat, !is.na(aad_unit))) # 6909
nrow(subset(dat, !is.na(aad_value))) # 6909
...
foo <- dat %>%
  ...
  mutate(earliestdob = ymd(sprintf("%04d-%02d-01", q220y, q220m)),
         dod = earliestdob + aadd) %>%
  filter(dod > qintdate) %>% pull(recnr)
...
id_drop2 <- dat %>%
  ...
  filter(dod > qintdate)
nrow(id_drop2) # 1
dat <- subset(dat, !(recnr %in% id_drop2$recnr))
nrow(dat) # 79693

nrow(subset(dat, !is.na(aad_unit))) # 6908, 1 less than 6914
nrow(subset(dat, !is.na(aad_value))) # 6908, 1 less than 6914
nrow(subset(dat, !is.na(aadd))) # 6908
nrow(subset(dat, !is.na(aadm))) # 6908
nrow(subset(dat, !is.na(aady))) # 6908
...
id_drop3 <- dat %>%
  ...
  filter(dod > qintdate)
nrow(id_drop3) # 2
dat <- subset(dat, !(recnr %in% id_drop3$recnr))
nrow(dat) # 79691

nrow(subset(dat, !is.na(aad_unit))) # 6906, 2 less than 6908
nrow(subset(dat, !is.na(aad_value))) # 6906, 2 less than 6908
nrow(subset(dat, !is.na(aadd))) # 6906
nrow(subset(dat, !is.na(aadm))) # 6906
nrow(subset(dat, !is.na(aady))) # 6906
table(dat$event, useNA = "always") # 6906
...
saveRDS(dat, "./gen/combined/temp/fph-clean-aod.rds")
