...
write.csv(reach_femme_qsecover, paste0(mypath, "reach_femme_qsecover.csv"), row.names=F)
write.csv(reach_femme_qwsec01, paste0(mypath, "reach_femme_qwsec01.csv"), row.names=F)
write.csv(reach_femme_qwsec2a, paste0(mypath, "reach_femme_qwsec2a.csv"), row.names=F)
write.csv(reach_femme_qwsec2b, paste0(mypath, "reach_femme_qwsec2b.csv"), row.names=F)
write.csv(reach_femme_qwsec2c, paste0(mypath, "reach_femme_qwsec2c.csv"), row.names=F)
write.csv(reach_femme_qwsec6a, paste0(mypath, "reach_femme_qwsec6a.csv"), row.names=F)
write.csv(reach_femme_qwsec11, paste0(mypath, "reach_femme_qwsec11.csv"), row.names=F)
write.csv(reach_femme_qwsec2y, paste0(mypath, "reach_femme_qwsec2y.csv"), row.names=F)
...
reach_mortalite_denomb_qhhsecover <- dbGetQuery(db,
                                          "SELECT * FROM qhhsecover") %>% rename_with(tolower) %>%
  rename_with(~ gsub("-", "_", .x))
print("reach_mortalite_denomb_qhhsecover downloaded")
print(Sys.time())

reach_mortalite_denomb_qhsec01xx <- dbGetQuery(db,
                                                "SELECT * FROM qhsec01xx") %>% rename_with(tolower) %>%
  rename_with(~ gsub("-", "_", .x))
print("reach_mortalite_denomb_qhsec01xx downloaded")
print(Sys.time())

reach_mortalite_denomb_section_che <- dbGetQuery(db,
                                               "SELECT * FROM section_che") %>% rename_with(tolower) %>%
  rename_with(~ gsub("-", "_", .x))
print("reach_mortalite_denomb_section_che downloaded")
print(Sys.time())

reach_mortalite_denomb_section_ct <- dbGetQuery(db,
                                                 "SELECT * FROM section_ct") %>% rename_with(tolower) %>%
  rename_with(~ gsub("-", "_", .x))
